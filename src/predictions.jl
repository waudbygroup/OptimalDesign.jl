...
For scalar models, returns a `Matrix{Float64}` of size `(n_samples, length(x_grid))`.
...
function posterior_predictions(
    ...
    y0 = prob.predict(first(particles), first(x_grid))

    if y0 isa Real
        # Scalar model → single Matrix
        predictions = Matrix{Float64}(undef, n_samples, length(x_grid))
        for (j, x) in enumerate(x_grid)
            for i in 1:n_samples
                predictions[i, j] = prob.predict(particles[i], x)
            end
        end
        return predictions
    else
        ...
        n_out = length(y0)
        preds = [Matrix{Float64}(undef, n_samples, length(x_grid)) for _ in 1:n_out]
        for (j, x) in enumerate(x_grid)
            for i in 1:n_samples
                y = prob.predict(particles[i], x)
                for c in 1:n_out
                    preds[c][i, j] = y[c]
                end
            end
