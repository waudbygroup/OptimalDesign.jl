"""
    ...
    plot_credible_bands(prob, result1, result2; labels=["Optimal", "Uniform"], truth=θ_true)

Plot credible bands from one or more experiment results. The prior from the first
result is shown as the first panel, followed by each result's posterior with its
observations overlaid.

The prediction grid is inferred from observations by default. Pass `x_grid` to override.
"""
function plot_credible_bands(
    prob::AbstractDesignProblem,
    results::AbstractExperimentResult...;
    x_grid::Union{Nothing,AbstractVector}=nothing,
    labels::Union{Nothing,AbstractVector{<:AbstractString}}=nothing,
    truth=nothing,
    kwargs...,
)
    all_obs = vcat((r.observations for r in results)...)
    grid = x_grid !== nothing ? x_grid : _default_grid(all_obs)

    posteriors = Particles[first(results).prior, (r.posterior for r in results)...]
    obs_list = Any[nothing, (r.observations for r in results)...]

    if labels !== nothing
        ls = String["Prior", labels...]
    else
        n = length(results)
        if n == 1
            ls = ["Prior", "Posterior"]
        else
            ls = String["Prior", ("Result $i" for i in 1:n)...]
        end
    end

    plot_credible_bands(prob, posteriors, grid;
        labels=ls, observations=obs_list, truth=truth, kwargs...)
end

"""Generate a dense 1D grid from observations, spanning the range of the first continuous field."""
function _default_grid(observations::AbstractVector{<:NamedTuple})
    isempty(observations) && error("Cannot infer grid from empty observations")
    x1 = first(observations).x
    # Find the first floating-point field (skip discrete/integer fields)
    x_field = nothing
    for k in keys(x1)
        if getfield(x1, k) isa AbstractFloat
            x_field = k
            break
        end
    end
    x_field === nothing && (x_field = first(keys(x1)))
    vals = [getfield(o.x, x_field) for o in observations]
    lo, hi = extrema(vals)
    margin = max(0.05 * (hi - lo), 1e-6)
    [(; x_field => v) for v in range(lo - margin, hi + margin, length=200)]
end

"""
    plot_credible_bands(prob, posterior, x_grid; level=0.9, observations=nothing)
...
function plot_credible_bands(
    ...
    x_field::Symbol=first(keys(first(x_grid))),
    n_samples::Int=200,
)
    preds = posterior_predictions(prob, posterior, x_grid; n_samples=n_samples)
    band = credible_band(preds; level=level)

    x_vals = [getfield(x, x_field) for x in x_grid]
    ...
    if observations !== nothing
        obs_x = [getfield(o.x, x_field) for o in observations]
...
"""
    plot_credible_bands(prob, posteriors, x_grid; labels, colors, truth, observations, ...)
...
- `x_field::Symbol`: which field of x to use as x-axis
...
function plot_credible_bands(
    ...
    y_true = if truth === nothing
        nothing
    elseif truth isa AbstractVector{<:Real} && length(truth) == length(x_grid)
        truth
    else
        # Assume truth is a parameter vector/NamedTuple — compute predictions
        [prob.predict(truth, x) for x in x_grid]
    end

    # Detect whether model is vector-valued
    test_preds = posterior_predictions(prob, first(posteriors), x_grid; n_samples=2)
    ...
    for (i, post) in enumerate(posteriors)
        preds = posterior_predictions(prob, post, x_grid; n_samples=n_samples)
        ...
        for comp in 1:n_components
            ...
            if obs !== nothing
                obs_x = [getfield(o.x, x_field) for o in obs]
                if is_vector
